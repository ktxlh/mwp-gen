...
class HSMM(nn.Module):
    ...
    def collapse_word_probs(self, row2tblent, wrd_dist, corpus):
        ...
        first_seen = {}
        for i, (field, idx, wrd) in row2tblent.items():
    ...
    def temp_bs(self, corpus, ss, start_inp, exh0, exc0, srcfieldenc,
        ...
        # at training time.
        inps = Variable(torch.LongTensor(K, 4), requires_grad=False)
            ...
            for k in range(2*K):
                ...
                if wrd == self.eop_idx and ell > 0:
                    ...
                    if wrd >= self.decoder.out_features: # a copy
                        ...
                        inps.data[len(new_hyps)].copy_(row2feats[int(tblidx)])   #:#KeyError: tensor(0)
                    else:
    ...
    def gen_one(self, templt, h0, c0, srcfieldenc, len_lps, row2tblent, row2feats):
        ...
        nout_wrds = self.decoder.out_features
        i2w, w2i = corpus.dictionary.idx2word, corpus.dictionary.word2idx
        #print(f"D gen_one len(templt)={len(templt)}") #0div
        for stidx, k in enumerate(templt):  #: state index, k?
            ...
            for ii in range(len(phrs_idxs)):
                if phrs_idxs[ii] < nout_wrds:
                    ...
                    _, _, wordstr = row2tblent[int(tblidx)] #:#KE
                    if args.verbose:
            ...
            if phrs[-1] == "<eos>":
                #print("\n\n\nbug here\n\n\n")
                break   #:#0div This caused ZeroDivisionError. 
                # tokes is set to 0
                # templt is (11,)           len(templt) == 1   
                # phrs == ["<eos>"]  and    phrs[-1] == "<eos>"
                # tokes is left 0
            phrases.append(phrs)
            tote_wscore += wscore
            tote_lscore += lscore
            ...
            for ell in range(self.L+1):
                ...
                for k in range(2*K):
                    ...
                    if wrd == self.eop_idx and ell > 0 and (not final_state or curr_hyps[anc][0] == eos_idx):
                        ...
                        if wrd >= self.decoder.out_features: # a copy
                            tblidx = wrd - self.decoder.out_features
                            inps.data[len(new_hyps)].copy_(row2feats[int(tblidx)]) #:#KE
                        else:
                            inps.data[len(new_hyps)][0] = wrd if i2w[wrd] in genset else unk_idx
...
if __name__ == "__main__":
    ...
    def test(epoch):
        ...
        for i in range(len(corpus.valid)):
            ...
            srcenc, srcfieldenc, uniqenc = net.encode(Variable(src, requires_grad=False),  # bsz x hid
                                                      Variable(amask, requires_grad=False),
                                                      Variable(uniqfields, requires_grad=False))
            init_logps, trans_logps = net.trans_logprobs(uniqenc, seqlen) # bsz x K, T-1 x bsz x KxK
            len_logprobs, _ = net.len_logprobs()
            fwd_obs_logps = net.obs_logprobs(Variable(inps, requires_grad=False), srcenc,
                                             srcfieldenc, Variable(fmask, requires_grad=False),
                                             Variable(combotargs, requires_grad=False),
                                             bsz) # L x T x bsz x K
    ...
    def label_train():
        ...
        for i in range(len(corpus.train)):
            ...
            fwd_obs_logps = net.obs_logprobs(Variable(inps, requires_grad=False), srcenc,
                                             srcfieldenc, Variable(fmask, requires_grad=False),
                                             Variable(combotargs, requires_grad=False), bsz) # LxTxbsz x K
            ...
            for b in range(bsz):
                ...
                for (start, end, label) in seqs[b]:
                    print("%s|%d" % (" ".join(words[start:end]), label),end=" ") #0div
                print()

    def gen_from_srctbl(src_tbl, top_temps, coeffs, src_line=None):
        """
        generate from source table (e.g. the wiki key-value pair)
        """
        ...
        h0, c0 = torch.tanh(inits[:, :inits.size(1)//2]), inits[:, inits.size(1)//2:] #:#2to3
        ...
        for row, (k, idx, wrd) in row2tblent.items():
            if k in w2i:
        ...
        for i in range(len(corpus.train)):
            ...
            for bidx in range(bsz):
                if tuple(labe for (start, end, labe) in seqs[bidx]) in top_temps:
                    ...
                    for (field, idx), wrd in src_tbl.items():
