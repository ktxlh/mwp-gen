"""
code from https://github.com/nyu-dl/bert-gen
"""
""" Try to generate from BERT """
#import ipdb as pdb # ipdb exports functions to access the IPython debugger. See https://pypi.org/project/ipdb/
import sys
import argparse
import torch
...
def masked_decoding(toks, seg_tensor, masks, model, tokenizer, selection_strategy):
    ...
    last = toks.copy() # for Gibbs
    for step_n, mask_id in enumerate(masks):
        
        # Gibbs
        if last != toks and "[MASK]" not in toks:
            print("Iter %2d: %s" % (step_n, " ".join(toks)))
        last = toks.copy()
        toks[mask_id] = "[MASK]"
...
def main(arguments):
    ...
    parser.add_argument("--bert_version", default="bert-base-cased",
                        choices=["bert-base-uncased", "bert-base-cased",
                                 "bert-large-uncased", "bert-large-cased"]) # uncased -> cased
